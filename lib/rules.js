...
const Dataset = {
  '@type': {
    clause: 'MUST include a "Dataset" and either a `RepositoryCollection` or `RepositoryObject`',
    validate: function ({ error, info, warn, results}, values, entity, crate) {
      ...
      if (types.has('RepositoryObject')) {
        if (type) {
          error('MUST NOT include both “RepositoryCollection” and “RepositoryObject”');
        } else {
          type = 'RepositoryObject';
        }
      }
      if (!type) {
        error('MUST include either “RepositoryCollection” or “RepositoryObject”');
        type = 'Common';
      }
      // Some overall rules
  ...
  publisher: {
    ...
    clause: 'MUST have a list of at least one Person or Organization item as author',
    validate() { }
  },
...
const RepositoryCollection = {
  ...
  modality: {
    clause: 'MAY have a `modality` property which SHOULD be a reference to one or more of the Language Data Commons Modality Terms: SpokenLanguage,  WrittenLanguage,  Song,  Gesture,  SignLanguage,  WhistledLanguage (this information may be summarisable from collection members)',
...
const RepositoryObject = {
  conformsTo: {
    ...
    validate: function ({ error }, values, entity, crate) {
      ...
      if (urls.has(CollectionProfileUrl)) error('MUST NOT have Collection profile');
    } 
  },
  datePublished: Dataset.datePublished,
  license: Common.license,
      ...
      for (item of values) {
        ...
        for (let specialType of Object.keys(typesOfFile)) {
          ...
          if (!item.language) {
            ...
            info(`Language property not present on hasPart entity $(item["@id]) - inherting from  $(entity["@id])`);
...
const PrimaryMaterial = {
  ...
  language: {
    clause: 'MUST a language property, or the RepositoryObject that is `partOf` MUST have a language property, referencing a Language item (language my be inhereted from the parent RepoObject)',
    ...
    // TODO -- look for hasDerived
      ...
      for (let val of values) {
        if (!val["@id"]) {
          ...
          if (!referencedItem) {
            info(`Property value does not resolve to another entity in this crate: ${JSON.stringify(val)}`);
          } else {
             // Check the type
             PrimaryMaterial["@type"].validate({results, error, warn, info}, referencedItem["@type"], referencedItem, null);
...
const Annotation = {
  ...
  annotationType: {
    clause: 'MAY have an `annotationType` property which SHOULD be a reference to one or more of the Language Data Commons Modality Terms: SpokenLanguage,  WrittenLanguage,  Song,  Gesture,  SignLanguage,  WhistledLanguage (this information may be summarisable from collection members)',
    validate({ error, info, warn, results }, values, entity, crate) { 
      if (!values || values.length === 0) {
        ...
        if (!val["@id"]) {
          ...
          if (!referencedItem) {
            info(`Property value does not resolve to another entity in this crate: ${JSON.stringify(val)}`);
             ...
             info(`Property value does resolve to another entity in this crate: ${JSON.stringify(val)}`);
...
const rules = module.exports = { Common, Dataset, RepositoryCollection, RepositoryObject, PrimaryMaterial, DerivedMaterial, Language, LanguageDataTypes, Annotation };
