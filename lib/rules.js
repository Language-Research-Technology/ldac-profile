...
const Common = {
  ...
  inLanguage: {
    clause:
      'MAY have a `inLanguage` property which is a reference to one or more Language items',
    validate: function ({ error, info, results }, values, entity, crate) {
      if (!values || !values.length) {
        info('Does not have a `inLanguage` property');
        return;
      }
      info('Does have a `inLanguage` property');
...
const RepositoryCollection = {
  ...
  communicationMode: {
    clause:
      'MAY have a `communicationMode` property which SHOULD be a reference to one or more of the Language Data Commons Communication Mode Terms: SpokenLanguage,  WrittenLanguage,  Song,  Gesture,  SignLanguage,  WhistledLanguage (this information may be summarisable from collection members)',
...
const Annotation = {
  ...
  annotationType: {
    clause:
      'MAY have an `annotationType` property which SHOULD be a reference to one or more of the Language Data Commons Communication Mode Terms: SpokenLanguage,  WrittenLanguage,  Song,  Gesture,  SignLanguage,  WhistledLanguage (this information may be summarisable from collection members)',
    validate({ error, info, warn, results }, values, entity, crate) {
      if (!values || values.length === 0) {
